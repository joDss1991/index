...
let baseLyr=null; // couche de base courante gérée par setBase()
...
// ── Couches superposées ──
const ovL={
  cad:L.tileLayer('https://data.geopf.fr/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=CADASTRALPARCELS.PARCELS&STYLE=bdparcellaire&FORMAT=image/png&TILEMATRIXSET=PM&TILEMATRIX={z}&TILEROW={y}&TILECOL={x}',{attribution:'© IGN',maxZoom:20,maxNativeZoom:19,keepBuffer:4,opacity:.75}),
  pac:L.tileLayer('https://data.geopf.fr/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=LANDUSE.AGRICULTURE.LATEST&STYLE=normal&FORMAT=image/png&TILEMATRIXSET=PM&TILEMATRIX={z}&TILEROW={y}&TILECOL={x}',{attribution:'© IGN RPG',maxZoom:20,maxNativeZoom:18,keepBuffer:4,opacity:.55}),
  sar:null,
  catnat:null
};
...
function setBase(name){
  ...
  // Supprimer l'ancienne couche de base (y compris la couche initiale)
  ...
  baseLyr.addTo(map);
  baseLyr.bringToBack();
  bringTop(); // réaffirmer l'ordre des couches superposées actives
  currentBase=name;
  curBase=name;
...
async function fetchDates(){
  ...
  const _prevDate0=dates.length>0?dates[0]:null; // pour détecter si la date la plus récente change
  ...
  for(let attempt=0;attempt<3;attempt++){
    try{
      ...
      if(dates.length>0){
        ...
        // Ne recharger la couche NDVI que si la date la plus récente a changé
        // (évite de recréer la couche WMS sur chaque zoom → flash visuel)
        if(dates[0]!==_prevDate0) applyDate(dates[0]);
      } else {
...
async function openSheet(lat,lng){
  ...
  setTimeout(()=>map.invalidateSize(),50);
    ...
    document.getElementById('shBody').innerHTML=
      ...
      // Héro : score global
      '<div class="sin-hero sin-hero--'+(riskScore>=70?'critical':riskScore>=40?'high':riskScore>=15?'medium':'low')+'">'+
        '<div class="sin-hero-kpi-wrap">'+
          '<div class="sin-hero-kpi-lbl">Score de risque</div>'+
          '<div class="sin-hero-score">'+riskScore+'<span>/100</span></div>'+
        '</div>'+
        '<div class="sin-hero-right">'+
          '<div class="sin-hero-label">'+riskLabel+'</div>'+
          '<div class="sin-hero-period">90 derniers jours</div>'+
        '</div>'+
      '</div>'+
      // Cards 2×2
      '<div class="sin-cards">'+
        '<div class="sin-card'+(gelJours>0?' sin-card--frost':'')+'">'+
          '<span class="sin-card-icon">🧊</span>'+
          '<div class="sin-card-val'+(gelJours>0?' sin-card-val--frost':'')+'">'+gelJours+'</div>'+
          '<div class="sin-card-lbl">Jours gel</div>'+
          '<div class="sin-card-sub'+(gelSevere>0?' sin-card-sub--alert':'')+'">'+(gelSevere>0?gelSevere+' j ≤-3°C':'aucun')+'</div>'+
        '</div>'+
        '<div class="sin-card'+(orageDates.length>0?' sin-card--storm':'')+'">'+
          '<span class="sin-card-icon">⛈</span>'+
          '<div class="sin-card-val'+(orageDates.length>0?' sin-card-val--storm':'')+'">'+orageDates.length+'</div>'+
          '<div class="sin-card-lbl">Épisodes vent</div>'+
          '<div class="sin-card-sub">'+(orageDates.length>0?'précip+rafales':'aucun')+'</div>'+
        '</div>'+
        '<div class="sin-card'+(parseFloat(deficit)>30?' sin-card--drought':'')+'">'+
          '<span class="sin-card-icon">☀️</span>'+
          '<div class="sin-card-val'+(parseFloat(deficit)>30?' sin-card-val--drought':'')+'">'+deficit+'</div>'+
          '<div class="sin-card-lbl">mm déficit</div>'+
          '<div class="sin-card-sub'+(parseFloat(deficit)>80?' sin-card-sub--alert':'')+'">'+secRisque+'</div>'+
        '</div>'+
        '<div class="sin-card'+(greleDates.length>0?' sin-card--hail':'')+'">'+
          '<span class="sin-card-icon">🌨</span>'+
          '<div class="sin-card-val'+(greleDates.length>0?' sin-card-val--hail':'')+'">'+greleDates.length+'</div>'+
          '<div class="sin-card-lbl">Épisodes grêle</div>'+
          '<div class="sin-card-sub">'+(greleDates.length>0?(greleDates.filter(g=>g.certain).length>0?greleDates.filter(g=>g.certain).length+' confirmé(s)':'estimés'):'aucun')+'</div>'+
        '</div>'+
      '</div>'+
      // Accordéon : Grêle
      '<div class="sin-detail'+(greleDates.length>0?' open':'')+'" id="sinDetGrele">'+
        '<div class="sin-detail-hdr" onclick="this.parentElement.classList.toggle(\'open\')">'+
          '<div class="sin-detail-hdr-left"><div class="sin-detail-ic sin-ic--hail">🌨</div><span class="sin-detail-name">Épisodes de grêle détectés</span></div>'+
          '<i class="fas fa-chevron-down sin-chevron"></i>'+
        '</div>'+
        '<div class="sin-detail-body">'+
          '<span class="sin-detail-note">Code WMO 96/99 + proxy : averses &gt;5mm avec rafales &gt;50 km/h</span>'+
          (greleDates.length>0
            ? greleDates.slice(0,8).map(g=>
                '<div class="sin-row"><span class="sin-row-k">'+g.d.split('-').slice(1).reverse().join('/')+'</span>'+
                '<span class="sin-row-v" style="color:'+(g.certain?'#ce93d8':'#ba68c8')+';">'+(g.certain?'⚠️ ':'~')+g.p+'mm · '+g.w+'km/h'+(g.s>0?' · '+g.s+'mm':'')+'</span></div>'
              ).join('')
            : '<div style="font-size:10px;color:#66bb6a;padding:6px 0;">✅ Aucun épisode de grêle détecté</div>'
          )+
          '<span class="sin-detail-note" style="margin-top:8px;">⚠️ = confirmé WMO · ~ = estimé par proxy</span>'+
          '<a href="https://keraunos.org/statistiques-orages/france.html" target="_blank" class="sin-ext-link"><i class="fas fa-external-link-alt"></i> Base Keraunos (orages France)</a>'+
        '</div>'+
      '</div>'+
      // Accordéon : Gel
      '<div class="sin-detail'+(gelJours>0?' open':'')+'" id="sinDetGel">'+
        '<div class="sin-detail-hdr" onclick="this.parentElement.classList.toggle(\'open\')">'+
          '<div class="sin-detail-hdr-left"><div class="sin-detail-ic sin-ic--frost">🧊</div><span class="sin-detail-name">Analyse gel</span></div>'+
          '<i class="fas fa-chevron-down sin-chevron"></i>'+
        '</div>'+
        '<div class="sin-detail-body">'+
          '<div class="sin-row"><span class="sin-row-k">Jours gel (≤0°C)</span><span class="sin-row-v" style="color:#64b5f6;">'+gelJours+' j</span></div>'+
          '<div class="sin-row"><span class="sin-row-k">Gel sévère (≤-3°C)</span><span class="sin-row-v" style="color:#ef5350;">'+gelSevere+' j</span></div>'+
          (gelDates.length>0
            ? '<div class="sin-tl">'+gelDates.map(g=>
                '<div class="sin-tl-item"><div class="sin-tl-dot" style="background:'+(g.t<=-5?'#ef5350':g.t<=-3?'#64b5f6':'#90caf9')+'"></div>'+
                '<span>'+g.d.split('-').slice(1).reverse().join('/')+' — '+g.t.toFixed(1)+'°C</span></div>'
              ).join('')+'</div>'
            : '<div style="font-size:10px;color:#66bb6a;padding:6px 0;">✅ Aucun gel sur 90 jours</div>'
          )+
        '</div>'+
      '</div>'+
      // Accordéon : Précipitations & Vent
      '<div class="sin-detail'+(orageDates.length>0?' open':'')+'" id="sinDetPrecip">'+
        '<div class="sin-detail-hdr" onclick="this.parentElement.classList.toggle(\'open\')">'+
          '<div class="sin-detail-hdr-left"><div class="sin-detail-ic sin-ic--storm">⛈</div><span class="sin-detail-name">Précipitations intenses & Vent</span></div>'+
          '<i class="fas fa-chevron-down sin-chevron"></i>'+
        '</div>'+
        '<div class="sin-detail-body">'+
          '<span class="sin-detail-note">Épisodes : précip &gt;15mm ET rafales &gt;60 km/h</span>'+
          (orageDates.length>0
            ? orageDates.slice(0,6).map(o=>
                '<div class="sin-row"><span class="sin-row-k">'+o.d.split('-').slice(1).reverse().join('/')+'</span>'+
                '<span class="sin-row-v" style="color:#ffb74d;">'+o.p+' mm · '+o.w+' km/h</span></div>'
              ).join('')
            : '<div style="font-size:10px;color:#66bb6a;padding:6px 0;">Aucun épisode détecté</div>'
          )+
          '<a href="https://www.georisques.gouv.fr/risques/catnat" target="_blank" class="sin-ext-link" style="margin-top:6px;"><i class="fas fa-external-link-alt"></i> Vérifier CatNat</a>'+
        '</div>'+
      '</div>'+
      // Accordéon : Bilan hydrique
      '<div class="sin-detail open" id="sinDetSec">'+
        '<div class="sin-detail-hdr" onclick="this.parentElement.classList.toggle(\'open\')">'+
          '<div class="sin-detail-hdr-left"><div class="sin-detail-ic sin-ic--drought">☀️</div><span class="sin-detail-name">Bilan hydrique — 90 jours</span></div>'+
          '<i class="fas fa-chevron-down sin-chevron"></i>'+
        '</div>'+
        '<div class="sin-detail-body">'+
          '<div class="sin-hydro-grid">'+
            '<div class="sin-hydro-cell sin-hydro-cell--prec">'+
              '<div class="sin-hydro-lbl" style="color:#64b5f6;">Cumul 30j</div>'+
              '<div class="sin-hydro-val" style="color:#64b5f6;">'+precTotal.toFixed(0)+'</div>'+
              '<div class="sin-hydro-unit" style="color:#64b5f6;">mm</div>'+
            '</div>'+
            '<div class="sin-hydro-cell sin-hydro-cell--etp">'+
              '<div class="sin-hydro-lbl" style="color:#ffa726;">ETP 30j</div>'+
              '<div class="sin-hydro-val" style="color:#ffa726;">'+etpTotal.toFixed(0)+'</div>'+
              '<div class="sin-hydro-unit" style="color:#ffa726;">mm</div>'+
            '</div>'+
            '<div class="sin-hydro-cell sin-hydro-cell--def" style="background:'+(parseFloat(deficit)>80?'rgba(198,40,40,.15)':'rgba(46,125,50,.15)')+';border-color:'+(parseFloat(deficit)>80?'rgba(198,40,40,.3)':'rgba(46,125,50,.3)')+';">'+
              '<div class="sin-hydro-lbl" style="color:'+secColor+';">Déficit</div>'+
              '<div class="sin-hydro-val" style="color:'+secColor+';">'+deficit+'</div>'+
              '<div class="sin-hydro-unit" style="color:'+secColor+';">mm</div>'+
            '</div>'+
          '</div>'+
          '<div class="sin-row"><span class="sin-row-k">Précipitations 90j</span><span class="sin-row-v" style="color:#64b5f6;">'+precTotal.toFixed(0)+' mm</span></div>'+
          '<div class="sin-row"><span class="sin-row-k">ETP 90j</span><span class="sin-row-v" style="color:#ffa726;">'+etpTotal.toFixed(0)+' mm</span></div>'+
          '<div class="sin-row" style="padding-top:8px;border-top:1px solid rgba(255,255,255,.1);margin-top:4px;">'+
            '<span class="sin-row-k" style="color:rgba(255,255,255,.7);font-weight:700;">Déficit (ETP−Précip)</span>'+
            '<span class="sin-row-v" style="font-size:15px;color:'+secColor+';">'+deficit+' mm</span>'+
          '</div>'+
          '<div style="margin-top:8px;">'+
            '<div style="display:flex;justify-content:space-between;font-size:9px;font-family:var(--mono);margin-bottom:4px;">'+
              '<span style="color:rgba(255,255,255,.35);">Risque sécheresse</span><span style="color:'+secColor+';font-weight:700;">'+secRisque+'</span>'+
            '</div>'+
            '<div class="sin-bar"><div class="sin-bar-fill" style="width:'+Math.min(parseFloat(deficit)/200*100,100)+'%;background:'+secColor+';"></div></div>'+
          '</div>'+
          '<div class="sin-row" style="margin-top:6px;"><span class="sin-row-k">Séquence sèche max</span><span class="sin-row-v">'+secConsecMax+' j</span></div>'+
        '</div>'+
      '</div>'+
      '</div></div>'+
      ...
      +'<div class="wx-section-title">Prévisions météo<span class="wx-section-date" id="wxForecastPeriod">14 jours</span></div>'
...
let _lastFetchCenter=''; // évite de refetcher si on zoome sans bouger
map.on('moveend',()=>{
  if(!ndviOpen) return;
  // Ne re-fetcher les dates que si le centre a réellement bougé (~1km)
  const c=map.getCenter();
  const centerKey=c.lat.toFixed(2)+','+c.lng.toFixed(2);
  if(centerKey===_lastFetchCenter) return;
  _lastFetchCenter=centerKey;
  clearTimeout(_fetchDatesTimer);
  _fetchDatesTimer=setTimeout(()=>{fetchDates();lastFetch=Date.now();},1500);
});
setInterval(async()=>{
