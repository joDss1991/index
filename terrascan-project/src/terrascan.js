...
async function openSheet(lat,lng){
  ...
  setTimeout(()=>map.invalidateSize(),50);
    ...
    const [geo,rpgWfs,wx]=await Promise.allSettled([
      ...
      (async ()=>{
        ...
        try{const r1=await fetch(url1);const d1=await r1.json();if(d1.features&&d1.features.length>0) return d1;}catch(e){}
        const dl2=0.0008;
        ...
        return await (await fetch(url2)).json();
      })(),
    ...
    try{
      ...
      const r=await fetch('https://archive-api.open-meteo.com/v1/archive?latitude='+lat+'&longitude='+lng+'&start_date='+sinDate.toISOString().split('T')[0]+'&end_date='+new Date().toISOString().split('T')[0]+'&daily='+sinParams+'&timezone=Europe/Paris');
      sinData=await r.json();
    }catch(e){}
...
function computeNdviScore(){
  // Utilise les 2 dates du split pour calculer la perte
  if(!_splitDateL||!_splitDateR||!dates.length) return null;
  // Les valeurs NDVI moyennes sont dans ndviMoyVal (date courante)
  // Pour un calcul réel il faudrait l'API Process — on utilise une estimation
  const moyEl = document.getElementById('ndviMoyVal');
  if(!moyEl) return null;
  const curMoy = parseFloat(moyEl.textContent);
  if(isNaN(curMoy)) return null;
